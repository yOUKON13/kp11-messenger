...
import { GetBackground, GetLanguage, GetSounds, GetTheme } from '../../../../store/reducers/Settings/SettingsSelector';
import { LoadSettings, SaveSettings, SettingsActions } from '../../../../store/reducers/Settings/SettingsReducer';
...
const SettingsWindow: React.FC<PropType> = function ({ isOpened, toggleOpen }) {
  ...
  function onFileSet(event: any) {
    if (event.target.files[0]) {
      dispatch(SettingsActions.setBackground(event.target.files[0].path));
    }
  }
    ...
    <MessageWindow toggleOpen={toggleOpen} isOpened={isOpened}>
      <div className="settings flex-container">
        <div className="flex-container settings__avatar">
          <img className="roundy-image" src={user?.avatar ? `${server}${user?.avatar}` : 'assets/avatar.png'} alt="" />
        </div>
        <div className="flex-container settings__user-data">
          <p className="text-overflow">{user?.name + ' ' + user?.surname}</p>
          <Link to="/profile" className="invisible-link animated-button">
            <i className="fa-solid fa-arrow-up-right-from-square" />
          </Link>
        </div>
        <div className="settings__block block flex-container">
          <i className="fa-solid fa-moon settings__block-left" />
          <p>Темная тема</p>
          <div className="settings__block-right">
            <Checkbox isChecked={theme} setChecked={setTheme} className="sliding-checkbox" />
          </div>
        </div>
        <div className="settings__block block flex-container">
          ...
          <p>Звук</p>
          <div className="settings__block-right">
            <Checkbox isChecked={sounds} setChecked={setSounds} className="sliding-checkbox" />
          </div>
        </div>
        <div className="settings__block block flex-container">
          <i className="fa-regular fa-globe settings__block-left" />
          <p>Язык</p>
          <button onClick={showLanguages} className="invisible-button animated-button settings__block-right">
            <i className="fa-regular fa-angle-right" />
          </button>
          <DroppingList isShowing={isLanguagesListShowing}>
            <div className="settings__language flex-container">
              <p>Русский</p>
              <RadioButton name="language" onChange={handleLanguageChange} value="russian" currentValue={language} />
            </div>
            <div className="settings__language flex-container">
              <p>English</p>
              <RadioButton name="language" onChange={handleLanguageChange} value="english" currentValue={language} />
            </div>
          </DroppingList>
        </div>
        <div className="settings__block block flex-container">
